-- Drop any rows the reconcile inserted with kind='orphan' before we
-- shrink the CHECK constraint, otherwise the rebuild insert would
-- violate it.
DELETE FROM backups WHERE kind = 'orphan';
