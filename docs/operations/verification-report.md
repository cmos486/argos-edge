...
| WAF sources agree (v1.3.39.0) | `waf-sources-agree.sh` | ✅ PASS | Prod after deploy: dashboard 260 / overview 260 / AppSec 260 / LAPI 266, 2 % (the 6 `crowdsec-appsec-outofband` alerts miss the `appsec-` prefix; fix queued). Four readings of "AppSec events, last 24 h" within 5 %: `dashboard/security` `waf_engines.appsec.events`, `security/overview` `appsec_hits_24h + appsec_bans_24h`, `appsec/metrics` `total_hits`, LAPI through `cscli alerts list` (kind=waf + the two appsec-* scenarios). Before on prod (1.3.38.5): dashboard 0 / overview 0 / appsec 260 / LAPI 266, FAIL |
