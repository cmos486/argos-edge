...
| PENDING: runs after the v1.3.42.0 deploy (rollup-agree) | 1 |
...
| Hourly rollup (v1.3.42.0) | `rollup-agree.sh` | PENDING on prod (probe PASS on the dense demo) | Dense demo, PHASE 0 probe with the same SQL inside the panel cgroup under dd IO pressure: 168 h backfill 2 m 15 s, `/api/hosts` p99 7.2 ms, 36 of 1,362 periods throttled, divergence 0 on a closed hour (3,031 rows, 57,931,839 bytes). Final image on the demo: 169 h backfill in 39.0 s after the boot purge, `/api/hosts` p99 5.7 ms (0 of 1,172 over 50 ms), `rollup-agree.sh` PASS (lag 0, 3,049 rows / 55,240,994 bytes / 5 classes equal). Prod result recorded here after the deploy | Boot log `rollup: fill done`; `rollup-agree.sh` PASS at tolerance 0 (rows, bytes, per-class) with job lag <= 2 h; then 48 h of 6 h drift checks at 0 before v1.3.42.1 |
