...
| v1.3.40 smoke rows (log pipeline, strip cursor) | 2 |
| v1.3.41 smoke rows (read pool) | 1 |
| v1.3.42 smoke rows (rollup agree) | 1 |
| **Total smoke scripts** | **29** |
| EFFECT-verified PASS against prod stack (panel binary v1.3.35 / v1.3.38.x / v1.3.39.x) | 24 |
| PASS partial on prod, 48 h reading pending (logs-pipeline; v1.3.40.3) | 1 |
| EFFECT-verified PASS after the v1.3.40.4 deploy (strip-cursor) | 1 |
| PASS gates 0-2 on prod after the v1.3.41.0 deploy; gate 3 FAIL CPU-bound, see v1.3.41.1 (read-pool) | 1 |
| v1.3.41.1 gate 3 FAIL on prod by the pinned-refresh overlap (known issue, v1.3.42.0); gates 0-2 PASS | (same script) |
| PASS on prod after the v1.3.42.0 deploy (rollup-agree); 48 h drift gate running | 1 |
...
| Log pipeline (v1.3.40.0-.3) | `logs-pipeline.sh` | ⚠ PASS partial, 48 h pending | Prod after v1.3.40.3: filtered families stored since start 0 / 0 (PASS), 1,088 rows excluded today; strip PENDING until the panel is 25 h old (0 rows older than 25 h carry raw already; 411k rows emptied on 2026-09-26, access raw 661 MB -> 95 MB); purge-path p99 FAIL (204 ms while 214 rows were stripped) for the reason recorded in the v1.3.40 planning doc 4.1: the pinned warm-set refresh blocks reads up to 0.5 s every 24 s on the single connection even with no purge (max 542 ms, 15/3,667 samples over 50 ms), so the gate needs the read pool (moved to 40.1). DB size at 48 h pending | Rows of the filtered families stored since the panel start <= 100; `raw` NULL on access rows older than raw_hours + 1 h (PENDING until the panel is 25 h old); rows and DB size printed for the 48 h comparison; p99 of `/api/hosts` during `POST /api/logs/purge` <= 50 ms. Before on prod (1.3.39.1): 501,475 rows, 1.72 GB, 661.5 MB of access raw; 450 monitor rows and 64 health-checker rows stored in the first 20 min after a restart; purge 1.51 s with `/api/hosts` p99 1,246 ms |
| Strip cursor plan (v1.3.40.4) | `strip-cursor.sh` | PASS | Prod after v1.3.40.4 (2026-09-26 14:10 UTC): 18,003 rows stripped in 9.5 s (1,900 rows/s; v1.3.40.3 did 740 rows/s), panel CPU median 4.6 % max 4.8 %, `/api/hosts` p50 1.5 ms p99 5.8 ms max 32.6 ms on the single connection. Dense demo, 430k rows: strip 392 s -> 242 s, panel CPU 50-60 % -> 2-5 %, cursor SELECT 100+ ms -> 0.4 ms (`TestStripCursorPlan` pins the plan through the driver) | `raw_stripped` > 0 in the purge log line; panel CPU max <= 10 % during a strip of about 6 h of access rows; `/api/hosts` p50 / p99 / max reported (gated by the read pool smoke from v1.3.41.0) |
| Read pool (v1.3.41.0) | `read-pool.sh` | PASS gates 0-2; gate 3 FAIL CPU-bound, see 41.1 | Prod after v1.3.41.0 (2026-09-26 15:10 UTC): boot log `read pool enabled`; idle p99 5.1 ms (n=1,085), strip p99 3.8 ms, export x3 p99 884 ms with p50 2.4 ms (FAIL, threshold kept): all slow samples fall inside the slowest export while the panel sits at 99-102 % of its 1-CPU cap (CSV encoder vs handler on GOMAXPROCS=1, not locking); 10 min RSS 38-74 MB, WAL 7.2 MB stable. Dense demo, final image, dd IO pressure (host io avg10 mean 22 %, peak 44 %): idle 6.2 ms, strip of 427k rows 8.0 ms, export x3 13.2 ms, cap purge 15.5 ms; same image with `ARGOS_READ_POOL=0`: 79 / 648 / 219 / 14,661 ms | `/api/hosts` p99 <= 50 ms during idle (pinned refresh), a 6 h strip and three back-to-back 24 h exports (one aggregated p99); boot log mode matches `EXPECT_POOL`. IO pressure (dd loop) and the cap-purge gate are **demo only**: prod runs with `IO_PRESSURE=0` and `CAP_PURGE_ROWS=0` (no data deleted to pass a gate) |
| Export keyset + pace (v1.3.41.1) | `read-pool.sh` gate 3 | FAIL on prod by the pinned-refresh overlap (p99 63 ms; export alone p99 7.4 ms); known issue, closes in v1.3.42.0 | Prod after v1.3.41.1 (2026-09-26 16:03 UTC): 9 exports of 65,600 rows aggregated n=457, p50 1.5 ms, p95 7.1 ms, p99 63.0 ms, max 75.4 ms, 8 samples over 50 ms (v1.3.41.0: p99 884 ms); three timed exports right after: p99 7.4 ms, 0 of 180 over 50 ms, 8 throttled periods in 15 s. The slow samples coincide with the pinned 24 h refresh ticks on top of the export (`nr_throttled` +50 during the run). Dense demo: pool off p95 249 -> 24 ms, p99 395 -> 38 ms; pool on p99 9.8 ms (run a) and 67.7 ms (run b, same overlap) | `/api/hosts` p99 <= 50 ms over nine back-to-back 24 h exports (one aggregated p99), threshold unchanged; IO pressure and cap purge demo-only |
| Hourly rollup (v1.3.42.0) | `rollup-agree.sh` | PASS on prod; 48 h drift gate running | Dense demo, PHASE 0 probe with the same SQL inside the panel cgroup under dd IO pressure: 168 h backfill 2 m 15 s, `/api/hosts` p99 7.2 ms, 36 of 1,362 periods throttled, divergence 0 on a closed hour (3,031 rows, 57,931,839 bytes). Final image on the demo: 169 h backfill in 39.0 s after the boot purge, `/api/hosts` p99 5.7 ms (0 of 1,172 over 50 ms), `rollup-agree.sh` PASS (lag 0, 3,049 rows / 55,240,994 bytes / 5 classes equal). Prod after v1.3.42.0 (2026-09-26 16:51 UTC): migration 034 applied at boot, 173 h backfilled in 55.8 s after the boot purge (slowest hour 432 ms), `/api/hosts` p99 7.7 ms meanwhile, `rollup-agree.sh` PASS (lag 0, 2,128 rows / 177,798,187 bytes / 5 classes equal); 48 h drift gate running | Boot log `rollup: fill done`; `rollup-agree.sh` PASS at tolerance 0 (rows, bytes, per-class) with job lag <= 2 h; then 48 h of 6 h drift checks at 0 before v1.3.42.1 |
