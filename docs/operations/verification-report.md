...
| Export keyset + pace (v1.3.41.1) | `read-pool.sh` gate 3 | PENDING on prod (demo: pool off PASS p99 38 ms; pool on 1 of 2 runs PASS) | Dense demo, 66k-row 24 h export x3: v1.3.41.0 pool off p95 249 ms p99 395 ms; v1.3.41.1 pool off p95 24 ms p99 38 ms (the two 300 ms samples left are the pinned refresh on the writer); pool on p99 9.8 ms (run a) and 67.7 ms (run b, pinned 24 h refresh on top of the export at 101 % CPU). Throttling during an export: 14 of 16 periods with keyset alone, 0 of 36 with the pause. Prod result recorded here after the deploy | `/api/hosts` p99 <= 50 ms during three back-to-back 24 h exports (aggregated), threshold unchanged |
