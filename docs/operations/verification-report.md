...
| EFFECT-verified PASS against prod stack (panel binary v1.3.35 / v1.3.38.x) | 23 |
| EFFECT-verified PASS against the demo stack only, pending prod after deploy (waf-sources-agree; v1.3.39.0) | 1 |
...
| WAF sources agree (v1.3.39.0) | `waf-sources-agree.sh` | ⏳ pending prod (PASS on demo `1.3.39.0-rc`) | Four readings of "AppSec events, last 24 h" within 5 %: `dashboard/security` `waf_engines.appsec.events`, `security/overview` `appsec_hits_24h + appsec_bans_24h`, `appsec/metrics` `total_hits`, LAPI through `cscli alerts list` (kind=waf + the two appsec-* scenarios). Before on prod (1.3.38.5): dashboard 0 / overview 0 / appsec 260 / LAPI 266, FAIL |
