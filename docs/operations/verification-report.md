...
| PENDING: runs after the v1.3.41.0 deploy (read-pool; PASS on the dense demo with IO pressure) | 1 |
...
| Read pool (v1.3.41.0) | `read-pool.sh` | PENDING on prod (PASS on the dense demo) | Dense demo, final image, dd IO pressure (host io avg10 mean 22 %, peak 44 %): `/api/hosts` p99 idle 6.2 ms, strip of 427k rows 8.0 ms, export x3 aggregated 13.2 ms, cap purge 15.5 ms; same image with `ARGOS_READ_POOL=0`: 79 / 648 / 219 / 14,661 ms (kill-switch flips the behaviour). Prod result recorded here after the smoke | `/api/hosts` p99 <= 50 ms during idle (pinned refresh), a 6 h strip and three back-to-back 24 h exports (one aggregated p99); boot log mode matches `EXPECT_POOL`. IO pressure (dd loop) and the cap-purge gate are **demo only**: prod runs with `IO_PRESSURE=0` and `CAP_PURGE_ROWS=0` (no data deleted to pass a gate) |
