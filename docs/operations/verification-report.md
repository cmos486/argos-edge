...
| PENDING: runs after the v1.3.40.4 deploy (strip-cursor) | 1 |
...
| Strip cursor plan (v1.3.40.4) | `strip-cursor.sh` | PENDING (runs after the deploy) | Dense demo, 430k rows: strip 392 s -> 242 s, panel CPU 50-60 % -> 2-5 %, cursor SELECT 100+ ms -> 0.4 ms (`TestStripCursorPlan` pins the plan through the driver). Prod result recorded here after the smoke | `raw_stripped` > 0 in the purge log line; panel CPU max <= 10 % during a strip of about 6 h of access rows; `/api/hosts` p50 / p99 / max reported (gated by the read pool smoke from v1.3.41.0) |
