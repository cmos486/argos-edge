// Static ISO 3166-1 alpha-2 -> numeric table.
//
// world-atlas' countries-110m.json keys each feature by the numeric
// ISO 3166-1 code ("840" for US, "724" for Spain...). Our backend
// ships alpha-2 codes ("US", "ES"), so we need a client-side bridge
// between the two. The list below is hand-copied from the canonical
// ISO 3166-1 registry (en.wikipedia.org/wiki/ISO_3166-1_numeric) --
// 249 entries covering every country world-atlas carries plus a few
// dependencies the topology omits (harmless: the map just skips
// codes that don't match).
//
// Intentionally NOT a dependency: a 2 KiB static table is cheaper
// than pulling i18n-iso-countries (~250 KiB) for one lookup.
//
// Topology uses 110m resolution which merges tiny territories into
// their parent state. When DB-IP returns a code for such a territory
// (e.g. "GI" Gibraltar) the choropleth will show zero hits for that
// country even if the data has some -- acceptable given the visual
// target.
...
export const ISO2_TO_NUMERIC: Record<string, string> = {
  ...
  VN: '704', VU: '548', WF: '876', WS: '882', XK: '983', YE: '887',
